...
class BamlAsyncClient:
    ...
    async def PredictPerturbEffect(self, pert: str,gene: str,
        baml_options: BamlCallOptions = {},
    ) -> types.PerturbEffect:
        # Check if on_tick is provided
        if 'on_tick' in baml_options:
            # Use streaming internally when on_tick is provided
            __stream__ = self.stream.PredictPerturbEffect(pert=pert,gene=gene,
                baml_options=baml_options)
            return await __stream__.get_final_response()
        else:
            # Original non-streaming code
            __result__ = await self.__options.merge_options(baml_options).call_function_async(function_name="PredictPerturbEffect", args={
                "pert": pert,"gene": gene,
            })
            return typing.cast(types.PerturbEffect, __result__.cast_to(types, types, stream_types, False, __runtime__))
...
class BamlStreamClient:
    ...
    def PredictPerturbEffect(self, pert: str,gene: str,
        baml_options: BamlCallOptions = {},
    ) -> baml_py.BamlStream[types.PerturbEffect, types.PerturbEffect]:
        __ctx__, __result__ = self.__options.merge_options(baml_options).create_async_stream(function_name="PredictPerturbEffect", args={
            "pert": pert,"gene": gene,
        })
        return baml_py.BamlStream[types.PerturbEffect, types.PerturbEffect](
          __result__,
          lambda x: typing.cast(types.PerturbEffect, x.cast_to(types, types, stream_types, True, __runtime__)),
          lambda x: typing.cast(types.PerturbEffect, x.cast_to(types, types, stream_types, False, __runtime__)),
          __ctx__,
        )
...
class BamlHttpRequestClient:
    ...
    async def PredictPerturbEffect(self, pert: str,gene: str,
        ...
        __result__ = await self.__options.merge_options(baml_options).create_http_request_async(function_name="PredictPerturbEffect", args={
            "pert": pert,"gene": gene,
        }, mode="request")
        return __result__
